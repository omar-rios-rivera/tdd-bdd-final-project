...
def get_products(product_id):
    """
    Reads a Product
    This endpoint will read a Product based on ID
    """
    app.logger.info("Looking for product with ID: %s", product_id)
    product = Product.find(product_id)
    if not product:
        return abort(status.HTTP_404_NOT_FOUND, f"Product with id '{product_id}' was not found.")
...
def update_products(product_id):
    """
    Updates a Product
    This endpoint will update a Product based on ID
    """
    app.logger.info("Updating product with ID: %s", product_id)
    check_content_type("application/json")
